"""Unit tests for the webhook action: validation, SSRF guard, retry."""
...
from app.scheduler.actions import WebhookError, _assert_safe_url, _call_webhook
...
async def test_webhook_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _allow_private(monkeypatch, True)
    calls = _mock_client(monkeypatch, lambda req: httpx.Response(200))
    action = WebhookAction(url="http://127.0.0.1/hook", max_retries=3)
    await _call_webhook(action, {"hello": "world"})
    assert calls["n"] == 1  # no retries needed


async def test_webhook_retries_then_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    _allow_private(monkeypatch, True)
    calls = _mock_client(monkeypatch, lambda req: httpx.Response(500))
